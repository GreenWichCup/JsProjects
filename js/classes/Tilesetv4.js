var numbWeap= prompt("Choisissez le nombre d'armes (1 à 6)");
var chooseFont =parseInt(prompt("Choisissez la fond de la carte. 1: Terre, 2: Herbe" ));
var xMap = 10;
var yMap = 6;
var min = 0;
var obstacleArray = [];
    ...
    drawObstacle(canvas) {
        ...
        while (obstacleArray.length < 8) {
            ...
            for (let i = 0; i < obstacleArray.length - 1; i++) {
                if (object[0] === obstacleArray[i][0] && object[1] === obstacleArray[i][1])
                    obstacleArray.pop(object);
            }
        }
        for (let k = 0; k < obstacleArray.length; k++) {
            canvas.drawImage(this.image, obstacleArray[k][0], obstacleArray[k][1], 70, 70);
        }
    }
    ...
    drawWeapon(canvas, k) {
        while (weaponArray.length < numbWeap) {
            ...
            for (let i = 0; i < obstacleArray.length; i++) {
                if (weapCoord[0] === obstacleArray[i][0] && weapCoord[1] === obstacleArray[i][1])
                    weaponArray.pop(weapCoord);
            };
            for (var j = 0; j < weaponArray.length - 1; j++) {
                if (weapCoord[0] === weaponArray[j][0] && weapCoord[1] === weaponArray[j][1]) {
                    weaponArray.pop(weapCoord);
                }
        ...
        canvas.drawImage(this.image, weaponArray[k][0] + 10, weaponArray[k][1] + 10, 50, 50);
    }
}
